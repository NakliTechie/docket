...
- Remote-target smoke (`SMOKE_BASE_URL`) mints its API token inside the compose app container (`docker compose exec`), with `SMOKE_API_TOKEN` override — a locally-minted token lives in the wrong database. Found on the first real compose run. 2026-06-10
- OIDC discovery against an explicitly `http://` issuer relaxes `SWD`/`WebFinger.url_builder` to `URI::HTTP` (`Sso.allow_http_discovery_for`) — swd hardcodes HTTPS regardless of issuer scheme, so the CI Keycloak could never be discovered; gated on the configured scheme so production https is untouched. 2026-06-10
- SSO buttons opt out of Turbo (`data-turbo=false`) — a Turbo fetch cannot follow the 302 to a cross-origin IdP; found by the first real-browser SSO run, invisible to OmniAuth test-mode. 2026-06-10
- CSP `form-action` allows the configured IdP origins (per-request lambda → `Sso.idp_form_action_origins`) — browsers enforce form-action against the redirect target of a form submission, so `'self'` alone silently blocked every SSO login in Chrome. 2026-06-10
- Keycloak test realm defines `openid`/`email`/`profile` client scopes explicitly with their mappers — a realm import does not auto-create built-ins when `clientScopes` is present, and Keycloak 26 hard-rejects unknown requested scopes (`invalid_scope`). 2026-06-10
- keycloak-sso CI job uploads `tmp/screenshots` on failure (`sso-screenshots` artifact) — the live-flow failures were undebuggable from logs alone. 2026-06-10
