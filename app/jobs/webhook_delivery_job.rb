...
class WebhookDeliveryJob < ApplicationJob
  ...
  def perform(delivery)
    ...
    # Resolve once and pin the connection to the vetted address while keeping
    # the original hostname for HTTP Host and TLS SNI. This closes the DNS
    # rebinding window between validation and connect.
    begin
      vetted_address = Docket::OutboundUrl.vetted_address(uri.host, port: uri.port)
    rescue Docket::OutboundUrl::Blocked => e
      delivery.update!(status: :failed, last_error: "blocked: #{e.message}".truncate(250))
      return
    end

    http = Net::HTTP.new(uri.host, uri.port)
    http.ipaddr = vetted_address
