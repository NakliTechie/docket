# PG8 — add/remove a contact's role on a deal. Mirrors DealCompetitorsController.
class DealContactRolesController < ApplicationController
  require_feature "crm"
  ...
  def create
    ...
    if @link.save
      ...
      redirect_to deal_path(@deal), alert: @link.errors.full_messages.to_sentence, status: :see_other
    end
  end
  ...
  def update
    ...
    if @link.update(link_params.except(:contact_id))
      ...
      redirect_to deal_path(@link.deal), alert: @link.errors.full_messages.to_sentence, status: :see_other
    end
  end
  ...
  def set_link
    @link = policy_scope(DealContactRole).find(params[:id])
  end
